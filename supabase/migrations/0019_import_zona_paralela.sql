...
-- 0019: import for the parallel bairro/zona model introduced in 0017.
--
-- Zona now hangs off the município (shared by every bairro it touches), and a
-- seção records BOTH its zona and its bairro, so the two dimensions roll up
-- independently. A row carrying a seção therefore needs zona AND bairro to be
-- placed; rows without a seção still import at zona, bairro or município
...
begin
  for v_idx in 0 .. jsonb_array_length(p_rows) - 1 loop
    ...
    begin
      ...
      -- Zona belongs to the município: shared across every bairro it touches.
