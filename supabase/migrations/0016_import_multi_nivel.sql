...
-- 0016: import at whatever granularity each file actually has.
--
-- Zona and Seção are now optional. Each row is recorded at the finest level it
-- provides, and the hierarchy above it is still created so the território
-- shows up in navigation:
--   municipio + bairro + zona + secao -> nivel 'secao'
--   municipio + bairro + zona         -> nivel 'zona'
--   municipio + bairro                -> nivel 'bairro'
--   municipio                         -> nivel 'municipio'
--
-- Rows are NOT deleted across grains: a campaign legitimately receives one
-- file per grain, and vw_votos_* (migration 0015) already resolves overlaps by
-- preferring the finest data, so both files coexist without double counting.
...
begin
  for v_idx in 0 .. jsonb_array_length(p_rows) - 1 loop
    ...
    begin
      ...
      -- Seção without zona can't be placed in the hierarchy.
      if v_secao_txt is not null and v_zona_txt is null then
        raise exception 'Seção informada sem zona correspondente';
      end if;
      -- Zona/seção hang off a bairro, so a bairro is required to place them.
      if v_zona_txt is not null and v_bairro_nome is null then
        raise exception 'Zona informada sem bairro correspondente';
      end if;
      ...
      -- Record at the finest level this row actually carries.
